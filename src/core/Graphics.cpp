...
void Graphics::DrawSprite(float xPos, float yPos, float width, float height)
{
    ...
    Vertex vertices[] = {
            ...
            //Positions								            //Colors						    //Texture Coordinates	//TexID
            Vertex(glm::vec3((-0.5f + xPos) * width, (-0.5f + yPos) * height, 0),	glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 	glm::vec2(1.0f, 1.0f), 	0.0f),
            Vertex(glm::vec3( (0.5f + xPos) * width, (-0.5f + yPos) * height, 0),	glm::vec4(1.0f,	1.0f, 1.0f, 1.0f), 	glm::vec2(1.0f, 0.0f), 	0.0f),
            Vertex(glm::vec3( (0.5f + xPos) * width,  (0.5f + yPos) * height, 0),	glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 	glm::vec2(0.0f, 0.0f), 	0.0f),
            Vertex(glm::vec3((-0.5f + xPos) * width,  (0.5f + yPos) * height, 0),    glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 	glm::vec2(0.0f, 1.0f), 	0.0f)
        };
